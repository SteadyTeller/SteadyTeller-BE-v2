...
public class LearningTaskAiService {
	...
	public List<AiGeneratedTaskDto> generateTasks(MemberGoal goal) {
		return generateTasks(goal, "가용시간 정보 없음");
	}

	public List<AiGeneratedTaskDto> generateTasks(MemberGoal goal, String constraint) {
		try {
			List<AiGeneratedTaskDto> tasks = chatClient.prompt().user(buildPrompt(goal, constraint)).call().entity(new ParameterizedTypeReference<List<AiGeneratedTaskDto>>() {
			});
	...
	private boolean invalid(AiGeneratedTaskDto t) {
		return t == null || blank(t.title()) || blank(t.category()) || blank(t.subject()) || t.difficulty() == null || t.difficulty() < 1 || t.difficulty() > 5 || t.allocatedMinutes() == null || t.allocatedMinutes() <= 0 || t.allocatedMinutes() > MAX_ALLOCATED_MINUTES;
	}
	...
	private String buildPrompt(MemberGoal goal, String constraint) {
		return """
